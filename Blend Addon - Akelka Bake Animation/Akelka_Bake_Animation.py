bl_info = {
    ...
    "version": (1, 6, 0),
    "blender": (4, 0, 0),
    "location": "View3D > Sidebar (N) > Akelka Tools > Bake Animation",
...
class BAC_Properties(PropertyGroup):
    use_scene_range: BoolProperty(name="Scene", default=False)
    ...
    auto_select_bones: BoolProperty(
        name="All Bones",
        description="Before bake, select all bones except Dummy_Root",
        default=True,
    )
...
class BAC_PT_main(Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.prop(props, "auto_select_bones", toggle=True, icon="BONE_DATA")
        ...
        sub.prop(props, "frame_step", text="")

        box = layout.box()
        _toggle_row(box, props, "only_selected", "visual_keying")
        _toggle_row(box, props, "clear_constraints", "clear_parents")
        _toggle_row(box, props, "overwrite_action", "clean_curves")
        _toggle_row(box, props, "clear_drivers", "clear_nla")
        _toggle_row(box, props, "bake_pose", "bake_object")
        _toggle_row(box, props, "channel_location", "channel_rotation")
        _toggle_row(box, props, "channel_scale", "channel_bbone")
