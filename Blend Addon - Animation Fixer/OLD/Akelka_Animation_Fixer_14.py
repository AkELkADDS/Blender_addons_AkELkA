...
def register_props():
    ...
    sc.show_old_method = bpy.props.BoolProperty(
        name="Old Method",
        description="Show the Old Method tools",
        default=False,
    )
    ...
    sc.aaf_ik_quat_fix = bpy.props.BoolProperty(
        name="IK Quat Fix", default=True,
        description="Fix quaternion flip spikes on Dummy_*_IK targets when setting up IK",
    )
    sc.aaf_ik_quat_spike_deg = bpy.props.FloatProperty(
        name="Spike",
...
def unregister_props():
    for p in ("show_old_method", "show_advanced",
...
def quat_make_continuous(new_q, ref_q):
    ...
    if ref_q is not None and new_q.dot(ref_q) < 0.0:
        new_q.negate()
...
def larian_ik_target_bone_names(arm_obj):
    """Dummy_*_IK target bones used by the 4-limb IK setup."""
    names = []
    for spec in resolve_larian_ik_limbs(arm_obj):
        target = spec.get('target')
        if target and arm_obj.pose.bones.get(target):
            names.append(target)
    return names

def _ikqf_action_for_armature(arm_obj):
...
def _ikqf_rotation_fcurves(action, bone_name):
    prefix = f'pose.bones["{bone_name}"].'
    out = []
    for fcu in action.fcurves:
        if not fcu.data_path.startswith(prefix):
            continue
        if 'rotation_quaternion' in fcu.data_path or 'rotation_euler' in fcu.data_path:
            out.append(fcu)
    return out

def _ikqf_bone_rotation_paths(bone_name):
    return (
        f'pose.bones["{bone_name}"].rotation_quaternion',
        f'pose.bones["{bone_name}"].rotation_euler',
    )

def _ikqf_quat_fcurves(action, bone_name):
    path, _ = _ikqf_bone_rotation_paths(bone_name)
    channels = []
    for i in range(4):
        fcu = action.fcurves.find(path, index=i)
        if fcu is None:
            return None
        channels.append(fcu)
    return channels

def _ikqf_euler_fcurves(action, bone_name):
    _, path = _ikqf_bone_rotation_paths(bone_name)
    channels = []
    for i in range(3):
        fcu = action.fcurves.find(path, index=i)
        if fcu is None:
...
def _ikqf_resolve_rotation_channels(action, arm_obj, bone_name):
    pb = arm_obj.pose.bones.get(bone_name)
    if not pb:
        return None, None
    quat_fcurves = _ikqf_quat_fcurves(action, bone_name)
    if pb.rotation_mode == 'QUATERNION' and quat_fcurves:
        return 'QUATERNION', quat_fcurves
    euler_fcurves = _ikqf_euler_fcurves(action, bone_name)
    if euler_fcurves:
        return pb.rotation_mode, euler_fcurves
    if quat_fcurves:
        return 'QUATERNION', quat_fcurves
    return None, None

def _ikqf_evaluate_quat(quat_fcurves, frame):
    q = Quaternion((
        quat_fcurves[0].evaluate(frame),
        quat_fcurves[1].evaluate(frame),
        quat_fcurves[2].evaluate(frame),
        quat_fcurves[3].evaluate(frame),
    ))
    if q.dot(q) > 1e-12:
        q.normalize()
    return q

def _ikqf_integer_frames(fcurves):
    if not fcurves:
        return []
    return sorted(set(int(round(kp.co[0])) for kp in fcurves[0].keyframe_points))

def _ikqf_clear_rotation_keys(action, bone_name):
    for fcu in list(_ikqf_rotation_fcurves(action, bone_name)):
        action.fcurves.remove(fcu)

def _ikqf_build_sequence(channel_mode, channels, times):
    seq = []
    prev_q = None
    for t in times:
        frame = float(t)
...
def _ikqf_detect_spikes(seq, spike_deg):
    ...
    for i in range(1, len(seq) - 1):
        ...
        q_next = seq[i + 1][1]
        if (
            quat_delta_deg(q_prev, q) > thr
            and quat_delta_deg(q, q_next) > thr
            and quat_delta_deg(q_prev, q_next) <= thr
        ):
            spikes.append(i)
    return spikes
...
def _ikqf_fix_spikes(seq, spike_indices):
    ...
    for i in spike_indices:
        ...
        t, _q = seq[i]
        t_next, q_next = seq[i + 1]
        span = t_next - t_prev
        alpha = (t - t_prev) / span if abs(span) > 1e-9 else 0.5
...
def _ikqf_write_quat_sequence(action, bone_name, seq):
    path, _ = _ikqf_bone_rotation_paths(bone_name)
    _ikqf_clear_rotation_keys(action, bone_name)
    fcus = [action.fcurves.new(data_path=path, index=i) for i in range(4)]
    count = len(seq)
    for i, fcu in enumerate(fcus):
        fcu.keyframe_points.add(count)
        for j, (t, q) in enumerate(seq):
            fcu.keyframe_points[j].co = (float(t), (q.w, q.x, q.y, q.z)[i])
            fcu.keyframe_points[j].interpolation = 'BEZIER'
        fcu.update()
    return count

def _ikqf_write_euler_sequence(action, bone_name, seq, rotation_mode):
    _, path = _ikqf_bone_rotation_paths(bone_name)
    _ikqf_clear_rotation_keys(action, bone_name)
    fcus = [action.fcurves.new(data_path=path, index=i) for i in range(3)]
    count = len(seq)
    prev_euler = None
    eulers = []
    for _t, q in seq:
        eul = q.to_euler(rotation_mode)
        if prev_euler is not None:
            eul = euler_unwrap_continuous(eul, prev_euler, rotation_mode)
        prev_euler = eul
        eulers.append(eul)
    for i, fcu in enumerate(fcus):
        fcu.keyframe_points.add(count)
        for j, (t, _q) in enumerate(seq):
            fcu.keyframe_points[j].co = (float(t), float(eulers[j][i]))
            fcu.keyframe_points[j].interpolation = 'BEZIER'
        fcu.update()
    return count
...
_IKQF_SMOOTH_DELTA_DEG = 4.0


def _ikqf_frames_with_elevated_delta(seq, threshold_deg=_IKQF_SMOOTH_DELTA_DEG):
    """Frames that still have noticeable rotation steps (post-fix jitter zones)."""
    if len(seq) < 2:
        return []
    thr = float(threshold_deg)
    frames = []
    for i in range(1, len(seq)):
        d = quat_delta_deg(seq[i - 1][1], seq[i][1])
        if d > thr:
            frames.append(float(seq[i - 1][0]))
            frames.append(float(seq[i][0]))
    out = []
    seen = set()
    for t in frames:
        key = int(round(t))
...
def _ikqf_apply_local_gaussian_smooth(action, bone_name, center_frames, radius, factor):
    """
    Gaussian-smooth rotation f-curve keys around center frames (one Alt+S-style pass).
    Direct f-curve edit — does not rely on Graph Editor operator context.
    """
    if not center_frames:
        return 0
    frame_set = _ikqf_expand_frame_neighborhood(center_frames, radius)
    if not frame_set:
        return 0
...
def fix_ik_target_bone_rotation(
    ...
    fcurves = _ikqf_rotation_fcurves(action, bone_name)
    channel_mode, channels = _ikqf_resolve_rotation_channels(action, arm_obj, bone_name)
    smooth_centers = list(dict.fromkeys(spike_frames + flip_frames))
    if channels:
        times = _ikqf_integer_frames(fcurves)
        seq_post_fix = _ikqf_build_sequence(channel_mode, channels, times)
        smooth_centers = list(dict.fromkeys(
            smooth_centers + _ikqf_frames_with_elevated_delta(seq_post_fix)
        ))

    if smooth_centers:
        changed = _ikqf_apply_local_gaussian_smooth(
            action, bone_name, smooth_centers,
            _IKQF_LOCAL_SMOOTH_RADIUS, _IKQF_LOCAL_SMOOTH_FACTOR,
        )
        stats["smooth_frames"] = len(
            _ikqf_expand_frame_neighborhood(smooth_centers, _IKQF_LOCAL_SMOOTH_RADIUS)
        ) if changed else 0
        if verbose and changed:
            centers_s = ", ".join(str(int(round(t))) for t in smooth_centers[:12])
            more = "..." if len(smooth_centers) > 12 else ""
            print(f"[AAF] IK quat smooth {bone_name}: ±{_IKQF_LOCAL_SMOOTH_RADIUS} around [{centers_s}{more}]")
...
def apply_aaf_ik_quat_fix(arm_obj, sc, verbose=True, context=None):
    """Fix quaternion spikes on IK targets (selected bones, or all Dummy_*_IK)."""
    if not _aaf_ik_quat_fix_on(sc):
        return [], []
    spike_deg = float(getattr(sc, 'aaf_ik_quat_spike_deg', 45.0))
    bone_names = resolve_ik_quat_fix_bone_names(arm_obj, context)
    ok = []
    errors = []
...
def _apply_bezier_to_action(action):
    """Set all keyframes on an action to Bezier interpolation."""
    if not action or not action.fcurves:
        return 0
    changed = 0
    for fcu in action.fcurves:
...
def apply_bezier_interpolation_to_armature(arm_obj, verbose=False, context=None):
    """Set all keyframes on the armature animation to Bezier (NLA strip or action)."""
    if not arm_obj or arm_obj.type != 'ARMATURE':
        return 0
    if context is None:
        context = bpy.context

    action, strip, track_name = resolve_whole_smooth_source(arm_obj)
    if strip is not None:
        with nla_tweak_strip(context, arm_obj, strip) as tweak_action:
            changed = _apply_bezier_to_action(tweak_action)
        if verbose and changed:
            print(
                f"[AAF] Make Bezier: {changed} key(s) on '{arm_obj.name}' "
                f"NLA '{track_name}' action '{action.name}'"
            )
        return changed
    ...
    changed = _apply_bezier_to_action(action)
    if verbose and changed:
        print(f"[AAF] Make Bezier: {changed} key(s) on '{arm_obj.name}' action '{action.name}'")
    return changed

def apply_bezier_if_enabled(arm_obj, sc, verbose=False, context=None):
    if sc is not None and _aaf_auto_bezier_on(sc):
        return apply_bezier_interpolation_to_armature(arm_obj, verbose=verbose, context=context)
    return 0
...
class SCENE_OT_make_larian_good(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        apply_bezier_if_enabled(arm, sc, verbose=True, context=context)
...
class SCENE_OT_make_bezier(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not arm:
            self.report({'ERROR'}, "Select an armature.")
            return {'CANCELLED'}
        changed = apply_bezier_interpolation_to_armature(arm, verbose=True, context=context)
        if changed:
...
# -----------------------------
class SCENE_OT_toggle_old_method(bpy.types.Operator):
    bl_idname = "scene.toggle_old_method"
    bl_label = "Old Method"
    bl_description = "Show or hide Old Method tools"
    bl_options = {'INTERNAL'}

    def execute(self, context):
        sc = context.scene
        sc.show_old_method = not bool(sc.show_old_method)
        return {'FINISHED'}
...
class POSE_OT_bake_fast(bpy.types.Operator):
    ...
    def finish(self, context, success: bool):
        ...
        if success:
            apply_bezier_if_enabled(self.arm_obj, sc, verbose=True, context=bpy.context)
...
class VIEW3D_PT_pose_align_panel(bpy.types.Panel):
    ...
    def _draw_ik_toggles(self, layout, sc, context):
        ...
        self._draw_ik_quat_fix_row(layout, sc)
    ...
    def _draw_old_method_box(self, layout, context, sc):
        ...
        header = old_col.row(align=True)
        header.scale_y = 0.85
        header.alignment = 'LEFT'
        header.operator(
            "scene.toggle_old_method",
            text="Old Method:",
            icon='PRESET',
            emboss=False,
        )

        if not sc.show_old_method:
            return
