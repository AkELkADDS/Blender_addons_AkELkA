...
def fix_selected_objects_25_to_target(selected_objects, target_fps):
    ...
    return processed, f"Processed {processed} object(s) → {target_fps} FPS"
...
# ---------------------
# New UI and manual operators (simpler UX)
# ---------------------
class OBJECT_OT_ApplySceneFPS(Operator):
    bl_idname = "scene.akelka_apply_fps"
    bl_label = "Apply Scene FPS"
    bl_description = "Set scene FPS to the chosen target (30 or 60)"

    def execute(self, context):
        try:
            val = context.scene.akelka_target_fps
            context.scene.render.fps = int(val)
            self.report({'INFO'}, f"Scene FPS set to {val}")
            return {'FINISHED'}
        except Exception as e:
...
class OBJECT_OT_FixSelectedAnimations(Operator):
    ...
    bl_label = "Fix Selected Animations"
    bl_description = "Scale selected objects' animations from 25 FPS to the chosen target and normalize frames"

    def execute(self, context):
        selected = list(context.selected_objects)
        try:
            target_fps = int(context.scene.akelka_target_fps)
        except Exception:
            target_fps = 30
        processed, msg = fix_selected_objects_25_to_target(selected, target_fps)
        if processed > 0:
...
# ---------------------
# UI Panels: parent + subpanel with clearer layout
# ---------------------
class PANEL_PT_Akelka_BG3Animation(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # Import auto-fix checkbox (keeps behavior consistent)
        layout.prop(context.scene, "bool_isAnimation", text="Import: Treat as Animation (auto-fix)")

        # Target FPS switcher (radio-like buttons, easy to read)
        layout.label(text="Target FPS for fixes:")
        row = layout.row(align=True)
        row.prop(context.scene, "akelka_target_fps", expand=True)

        # Apply scene FPS and Fix Selected buttons use the chosen target fps
        row = layout.row(align=True)
        row.operator("scene.akelka_apply_fps", text="Apply Scene FPS")
        row = layout.row(align=True)
        row.operator("object.akelkafix_selected_animations", text="Fix Selected Animations")

        layout.separator()
        layout.label(text="Notes:")
        layout.label(text="Use 'Noesis FBX Import' subpanel to import files.")
...
def register():
    ...
    # target fps switch (string values '30' or '60' make UI simpler)
    bpy.types.Scene.akelka_target_fps = EnumProperty(
        name="Target FPS",
        description="Target FPS for auto-fix / manual fix",
        items=[
            ('30', "30 FPS", "Fix to 30 frames-per-second"),
            ('60', "60 FPS", "Fix to 60 frames-per-second"),
        ],
        default='30'
    )
...
def unregister():
    ...
    try:
        ...
        del bpy.types.Scene.akelka_target_fps
    except Exception:
        pass
