...
# ---------------------
# rotateObjectEachFrame (final patch: integer frames only)
# ---------------------
def rotateObjectEachFrame(obj):
    """
    Insert rotation keys only on valid, integer frames inside the clamped animation range.
    Ensures:
      - No fractional frames
      - No frames beyond the actual end frame
    """
    if obj is None:
        return
    ...
    # 1) Collect frames from action + NLA
    try:
    ...
    # Convert to integers by flooring to avoid fractional 900.25
    int_frames = set()
    for f in frames:
        try:
            int_f = int(math.floor(f + 1e-8))
            int_frames.add(int_f)
        except Exception:
            pass
    ...
    # Determine max frame (integer)
    max_frame = max(int_frames)

    # Remove any frames greater than max_frame (defensive)
    int_frames = {f for f in int_frames if f <= max_frame}

    if not int_frames:
        return

    # Insert rotation keys only on clean integer frames
...
# ---------------------
# Animation FPS fix + clamp + normalize logic
# ---------------------
...
def gather_original_frames_for_object(obj):
    """Gather keyframe frames from object's action and NLA strips BEFORE scaling."""
    frames = set()
    if obj is None:
        return frames
    try:
...
def clamp_action_to_max_frame_and_normalize(action, max_frame):
    """Remove keyframes > max_frame and floor remaining key times to integers (also normalize handles)."""
    if not action:
        return
    for fcurve in action.fcurves:
        # remove keys greater than max_frame
        to_remove = [kp for kp in list(fcurve.keyframe_points) if kp.co.x > max_frame + 1e-8]
        for kp in to_remove:
            try:
                fcurve.keyframe_points.remove(kp)
            except Exception:
                pass
        # normalize remaining key times to integers (floor)
        for kp in fcurve.keyframe_points:
            try:
                new_x = float(int(math.floor(kp.co.x + 1e-8)))
                kp.co.x = new_x
                # move handles to integer as well (avoid fractional display)
                kp.handle_left.x = new_x
                kp.handle_right.x = new_x
            except Exception:
                pass
        try:
            fcurve.update()
...
def normalize_strip_frames(strip, max_frame):
    """Ensure strip frame_start/end are integers and <= max_frame."""
    try:
        # floor starts and ends, then clamp
...
def fix_imported_animation_timing_and_clamp(imported_objects, original_fps, post_import_scene_fps, do_fix):
    ...
    Steps:
      ...
      5) Floor all remaining key times to integers and normalize NLA strips.
    """
    if not do_fix:
        return
    ...
    # 3) Clamp to floored scaled max per-object and normalize to integers
    for obj in imported_objects:
        if obj is None:
            continue
        ...
        # clamp and normalize direct action keyframes
        if obj.animation_data and obj.animation_data.action:
            try:
                clamp_action_to_max_frame_and_normalize(obj.animation_data.action, max_scaled)
            except Exception:
                pass

        # clamp and normalize NLA strips and their actions
        if obj.animation_data and hasattr(obj.animation_data, "nla_tracks"):
            for track in obj.animation_data.nla_tracks:
                for strip in track.strips:
                    try:
                        normalize_strip_frames(strip, max_scaled)
                    except Exception:
                        pass
                    try:
                        if strip.action:
                            clamp_action_to_max_frame_and_normalize(strip.action, max_scaled)
                    except Exception:
                        pass
...
class OBJECT_OT_Noira_FBXImporter(Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        for current_file in self.files:
            ...
            # IMPORTANT: Do NOT run rotateObjectEachFrame here; we will run it AFTER scaling & clamping
