...
def apply_all_transforms(obj=None, loc=True, rot=True, scale=True):
    # Apply transforms for selected objects (keeps original behaviour)
    currentMode = 'OBJECT'
    try:
...
def fix_imported_animation_timing(imported_objects, original_fps, post_import_scene_fps, do_fix):
    ...
    # We only apply the automatic scaling when FBX importer set post_import_scene_fps to 25
    FBX_FPS = 25.0
    try:
    ...
    # Only auto-fix for the two cases you requested (30 and 60fps)
        ...
        for current_file in self.files:
            ...
            for obj in importedObjects:
                ...
                if obj.type == 'ARMATURE':
                    ...
                    # preserve original behaviour: create zero rotation keys each frame
                    try:
